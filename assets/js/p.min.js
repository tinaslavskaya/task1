    ...
    boxRoot = {
        _init: function () {
            setTimeout(function () {
                ...
                puedo && count <= intentos && (jQuery(this).hasClass("abierta") || (puedo = !1,
                    jQuery(".circle-loader").removeClass("load-complete"), 
                    jQuery(".checkmark").css("display", "none"), 
                    jQuery(this).addClass("abierta"), 
                    2 == count ? (jQuery(this).addClass("premiazo"), setTimeout(function () {
                    ...
                    jQuery("#p_modal2").modal(modalOptions), setTimeout(function () {
                        jQuery(".circle-loader").addClass("load-complete"), 
                        jQuery(".checkmark").css("display", "block"), puedo = !0
                    }, 1e3)
                }, 2e3))))
...
let arra = [];
$('.survey_button').on('click', function(e) {
     let targ = $(e.target).text();
     let paren = $(e.target).parent();
     let child = paren.children('.question').children('span').text();
     let res = child.trim() + ' ' + targ.trim() ;
     arra += res  + '\n';  
});
